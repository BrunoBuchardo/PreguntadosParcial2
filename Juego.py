...
def seleccionar_opcion(opcion, datos_juego, incorrecto, correcto):
    """
    ...
    Args:
        ...
        incorrecto (pygame.mixer.Sound): Sonido para respuesta incorrecta.
        correcto (pygame.mixer.Sound): Sonido para respuesta correcta.
    
    Returns:
        dict: Estado actualizado del juego.
    """
    datos_juego = verificar_respuesta(opcion, datos_juego, incorrecto, correcto)
...
def mostrar_juego(pantalla, eventos, datos_juego, fuente, fuente_game_over):
    ...
    if not datos_juego["pregunta_actual"]:
        ...
        if tiempo_transcurrido > DIFICULTADES[datos_juego["dificultad_seleccionada"]]["tiempo_pregunta"]:
            ...
            datos_juego["mensaje_comodin_tiempo"] = pygame.time.get_ticks()
            if datos_juego["incorrecto"]:
                datos_juego["incorrecto"].play()
            datos_juego["toasty_active"] = True
            ...
            for i, opcion in enumerate(opciones_a_mostrar):
                dibujar_boton(pantalla, fuente, opcion, 200, 150 + i * 60, 400, 50, AMARILLO, AZUL, 
                              lambda opt=opcion: datos_juego.update(seleccionar_opcion(opt, datos_juego, datos_juego["incorrecto"], datos_juego["correcto"])))
            ...
            if not datos_juego["bomba_usada"]:
                dibujar_boton(pantalla, fuente, "Bomba", base_x_comodin + idx * (ancho_boton + espacio), base_y_comodin, ancho_boton, 50, AMARILLO, ROJO, 
                              lambda: datos_juego.update(usar_bomba(datos_juego), mensaje_comodin_tiempo=pygame.time.get_ticks()))
                idx += 1
            if not datos_juego["x2_usado"]:
                dibujar_boton(pantalla, fuente, "X2", base_x_comodin + idx * (ancho_boton + espacio), base_y_comodin, ancho_boton, 50, AMARILLO, ROJO, 
            ...
            if not datos_juego["doble_chance_usado"]:
                dibujar_boton(pantalla, fuente, "Doble Chance", base_x_comodin + idx * (ancho_boton + espacio), base_y_comodin, ancho_boton, 50, AMARILLO, ROJO, 
