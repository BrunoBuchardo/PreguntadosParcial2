...
def mostrar_menu(pantalla, eventos, datos_juego, fuente, fuente_game_over):
    ...
    botones_menu = [
        ("Jugar", AMARILLO, VERDE, lambda: datos_juego.update(estado=INICIO_JUEGO)),
        ("TOP 10 Partidas", AMARILLO, AZUL, lambda: datos_juego.update(estado=RANKINGS)),
        ("Configuración", AMARILLO, AZUL, lambda: datos_juego.update(estado=CONFIGURACION)),
        ("Opciones de Juego", AMARILLO, AZUL, lambda: datos_juego.update(estado=OPCIONES_DE_JUEGO, dificultad_seleccionada="MEDIO")),
        ("Salir", AMARILLO, ROJO, lambda: datos_juego.update(ejecutando=False)),
    ]
